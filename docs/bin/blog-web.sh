...
HEALTH_CHECK_URL="http://localhost:8443/actuator/health"
...
usage() {
    echo "用法: sh blog-web.sh [start|stop|restart|status|log|health|clean-log]"
    echo "  start    - 启动应用"
    echo "  stop     - 停止应用"
    echo "  restart  - 重启应用"
    echo "  status   - 查看状态"
...
# 日志轮转函数
rotate_logs(){
  local log_file="${LOG_DIR}/blog-web.log"
  if [[ -f "$log_file" ]]; then
